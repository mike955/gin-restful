...
#         Desc: tcp client connection pool
#       Author: ato.ye
#        Email: ato.ye@ucloud.cn
#     HomePage: http://www.ucloud.cn
#      Version: 0.0.1
#   LastChange: 2016-01-20 20:25:10
#      History:
=============================================================================*/
package ufnet
...
func newClientTcpConnection(s_peer_addr string, i_peer_port int, timeOut uint32) (c *clientTcpConnection, err error) {
	remote_addr := net.JoinHostPort(s_peer_addr, strconv.Itoa(i_peer_port))
	...
	if ok {
		...
		if connNum > 0 {
			...
			uflog.DEBUGF("get client connection [ %s -> %s ] from pool", c.tcpConn.conn.LocalAddr(), c.tcpConn.conn.RemoteAddr())
...
func connectServer(network, address string, timeout time.Duration) (c *clientTcpConnection, err error) {
	...
	uflog.DEBUGF("new client connection [ %s -> %s ]", conn.LocalAddr(), conn.RemoteAddr())
	...
	for i := 0; i < maxBadConnRetries; i++ {
		...
		if err == nil {
			...
			go func() {
				...
				//uflog.DEBUG("checking", len(res), res)
				readErrChan <- err
			}()
