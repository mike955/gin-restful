...
#      Version: 0.0.1
#   LastChange: 2016-01-20 20:26:13
#      History:
=============================================================================*/
package ufnet
...
func (s *tcpServer) serve() (err error) {
	uflog.DEBUGF("server start service at %s", s.listener.Addr())
	for {
		c, err := s.listener.Accept()
		if err != nil {
			uflog.ERRORF("Accept fail:%v", err)
			fmt.Println("Accept fail:", err)
			break
		}
		connection, err := s.newConnection(c)
		uflog.DEBUGF("new server connection [ %s -> %s ]", c.RemoteAddr(), c.LocalAddr())
...
func (s *tcpServer) serveConnection(c *TcpConnection) {
	for {
		req, err := c.Receive()
		if err != nil {
			uflog.DEBUGF("connection [ %s -> %s ] is closed", c.conn.RemoteAddr(), c.conn.LocalAddr())
			s.delConnection(c)
			return
		}
		// 每个请求启动一个goroutine处理
