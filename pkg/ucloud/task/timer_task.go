/*=============================================================================
#     FileName: timer_task.go
#         Desc: timer task
#       Author: ato.ye
#        Email: ato.ye@ucloud.cn
#     HomePage: http://www.ucloud.cn
#      Version: 0.0.1
#   LastChange: 2016-02-10 20:20:30
#      History:
=============================================================================*/
package uftask
...
func (t *TimerTask) Run() {
	...
	go func() {
		...
		if ufcommon.CheckWrapPanic() {
			ufcommon.GoSafeTimer(t.Handler.ServeTimer)
		} else {
...
func timerTaskServe() {
	...
	for tType, handle := range timerHandlePool {
		...
		if err != nil {
			uflog.ERRORF("create timer task[%d] fail", tType)
			continue
		}
		go task.Run()
		// 设置定时器
...
func runTimerTask(tType int32, handle *timerTaskHandle, t *time.Ticker) {
	for _ = range t.C {
		...
		task, err := newTimerTask(tType, handle)
		if err != nil {
			uflog.ERRORF("create timer task fail, type(%d)", tType)
			continue
		}
		go task.Run()
	}
